...
@test "remove-sa.sh should print the credentials" {
  ...
  mock_set_output "${mysql}" "mydogu_12345678" 1
  mock_set_output "${mysql}" "dontCare" 2
  ...
  assert_equal "$(mock_get_call_args "${mysql}" "1")" "-uroot -B --disable-column-names -e SHOW DATABASES like 'mydogu\_%'"
  assert_equal "$(mock_get_call_args "${mysql}" "2")" '-uroot -e DROP DATABASE if exists mydogu_12345678;'
  assert_equal "$(mock_get_call_args "${mysql}" "3")" '-uroot -e DROP USER if exists mydogu_12345678;'
